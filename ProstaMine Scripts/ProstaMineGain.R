#----Get Input List----
 gain.input <- read.table("Gain_Input_1.txt")
 gain.input <- gain.input$V1
 # Make .mae
...
#----Run ProstaMine for Gain Subtypes----
for (z in 1 :length(gain.input)){
  source("analysis scripts.R")
  #----Specify subtype and select filtering cutoffs----
  query.gene <- gain.input[z]
  ...
  WT.cutoff <- .3
  # Output Labels
  query.gene.alteration <- paste(query.gene, "Gain")
  query.gene.wt <- paste(query.gene, "WT")
  wt <- paste(query.gene,"WT Tumors")
  st <- paste(query.gene, "Gain Tumors")
  output.filename <- paste(query.gene, "Gain.RData", sep = "")
  ...
  # Primary
  primary.ST <- alteration_subset(primary.GOF, c(query.gene, "altered"))
  ...
  # Metastatic
  met.ST <- alteration_subset(met.GOF, c(query.gene, "altered"))
  ...
  TCGA.ST <- alteration_subset(TCGA.GOF, c(query.gene, "altered"))
  TCGA.WT <- alteration_subset(TCGA.alteration, c(query.gene, "unaltered"))
  # Taylor primary background for DGE
  Taylor.primary.ST <- alteration_subset(Taylor.primary.GOF, c(query.gene, "altered"))
  Taylor.primary.WT <- alteration_subset(Taylor.primary.GOF, c(query.gene, "unaltered"))
  # Taylor met background for DGE
  Taylor.met.ST <- alteration_subset(Taylor.met.GOF, c(query.gene, "altered"))
  Taylor.met.WT <- alteration_subset(Taylor.met.GOF, c(query.gene, "unaltered"))
  # Barbieri background for DGE
  Barbieri.ST <- alteration_subset(Barbieri.GOF, c(query.gene, "altered"))
  ...
  # Abida
  Abida.ST <- na.omit(alteration_subset(met.GOF, c(query.gene, "altered")))
  ...
  if ((length(Taylor.primary.patients) > 0) & ((length(Barbieri.patients)) > 0) &
    ...
    Taylor.ST <- alteration_subset(Taylor.primary.GOF, c(query.gene, "altered"))
